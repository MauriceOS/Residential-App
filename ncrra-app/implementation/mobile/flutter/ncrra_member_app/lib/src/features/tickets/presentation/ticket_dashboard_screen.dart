...
class TicketDashboardScreen extends StatefulWidget {
  const TicketDashboardScreen(
      {required this.onBack, required this.onNavigate, super.key});
  ...
  List<TicketItem> get _tickets {
    ...
    final result = previewTickets.where((ticket) {
      ...
      final queryMatches = query.isEmpty ||
          '${ticket.id} ${ticket.title} ${_serviceName(ticket.service)}'
              .toLowerCase()
              .contains(query);
    ...
    result.sort((a, b) {
      return switch (_sort) {
        ...
        _TicketSort.status =>
          _statusName(a.status).compareTo(_statusName(b.status)),
          ...
          children: [
            NcrraTopBar(
              ...
              action: IconButton(
                ...
                icon: Icon(
                    _filtersOpen ? LucideIcons.x : LucideIcons.list_filter),
            ...
            Expanded(
              child: ListView(
                padding: const EdgeInsets.fromLTRB(
                    NcrraSpacing.gutter, 0, NcrraSpacing.gutter, 24),
                children: [
                  TextField(
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(children: [
                      ...
                      _statusChip(
                          'Awaiting provider', TicketStatus.awaitingProvider),
                  ...
                  Row(children: [
                    ...
                    const Spacer(),
                    const Icon(LucideIcons.arrow_down_up,
                        size: 14, color: NcrraColors.mutedText),
                  ...
                  if (tickets.isEmpty)
                    ...
                    ...tickets.map((ticket) => Padding(
                        ...
                        child: _TicketCard(
                            ticket: ticket, onTap: () => _showDetail(ticket)))),
                ],
              ),
            ),
    ...
    return Padding(
      ...
      child: ChoiceChip(
        ...
        labelStyle: Theme.of(context).textTheme.labelMedium?.copyWith(
            color: selected ? NcrraColors.white : NcrraColors.secondaryText),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Service',
          style: Theme.of(context)
              .textTheme
              .labelMedium
              ?.copyWith(color: const Color(0xFF435166))),
      ...
      DropdownButtonFormField<TicketService?>(
        ...
        items: const [
          ...
          DropdownMenuItem<TicketService?>(
              value: TicketService.electricity, child: Text('Electricity')),
          DropdownMenuItem<TicketService?>(
              value: TicketService.water, child: Text('Water')),
          DropdownMenuItem<TicketService?>(
              value: TicketService.property, child: Text('Property')),
        ],
      ),
    ]);
  }
  ...
  Widget _sortSelect(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      DropdownButtonFormField<_TicketSort>(
        ...
        isExpanded: true,
        onChanged: (value) =>
            setState(() => _sort = value ?? _TicketSort.newest),
        items: const [
          DropdownMenuItem(
              value: _TicketSort.newest, child: Text('Newest first')),
          DropdownMenuItem(
              value: _TicketSort.oldest, child: Text('Oldest first')),
          ...
          const SizedBox(
              ...
              child: Align(
                  ...
                  child: Icon(LucideIcons.search,
                      size: 26, color: NcrraColors.teal))),
          const SizedBox(height: 12),
          Text('No matching tickets',
              style: Theme.of(context).textTheme.labelLarge),
          const SizedBox(height: 4),
          Text('Try a different service, status or search term.',
                ...
                children: [
                  Center(
                      child: Container(
                          ...
                          height: 5,
                          decoration: const BoxDecoration(
                              color: Color(0xFFD7DCDB),
                              borderRadius:
                                  BorderRadius.all(NcrraRadius.pill)))),
                  const SizedBox(height: 20),
                  NcrraStatusBadge(
                      label: _statusName(ticket.status),
                  ...
                  const Divider(),
                  const SizedBox(height: 12),
                  _timeline('Request submitted', 'NCRRA recorded your request.',
                      true),
                  _timeline('Current: ${_statusName(ticket.status)}',
                      'NCRRA will notify you when this changes.', false),
                  _timeline(
                      'Member confirmation',
                      'You can review the outcome when the ticket closes.',
                      false),
                ]),
          ),
        );
  ...
  Widget _timeline(String title, String body, bool complete) {
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        NcrraIcon(
            icon: complete ? LucideIcons.check : LucideIcons.minus,
            tone: NcrraStatusTone.teal),
        const SizedBox(width: 12),
        Expanded(
            child:
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          NcrraIcon(
              icon: _icon(ticket.service),
              tone: ticket.status == TicketStatus.awaitingProvider
                  ? NcrraStatusTone.amber
                  : NcrraStatusTone.teal),
          const SizedBox(width: 12),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                ...
                const SizedBox(height: 5),
                Text('${_serviceName(ticket.service)} · ${ticket.updatedLabel}',
                    style: Theme.of(context).textTheme.bodySmall),
              ])),
...
extension _TicketSortLabel on _TicketSort {
  String get label => switch (this) {
        _TicketSort.newest => 'Newest',
        _TicketSort.oldest => 'Oldest',
        _TicketSort.status => 'Status'
      };
}

String _statusName(TicketStatus status) => switch (status) {
      TicketStatus.awaitingProvider => 'Awaiting provider',
      TicketStatus.inProgress => 'In progress',
      TicketStatus.resolved => 'Resolved',
      TicketStatus.closed => 'Closed'
    };
String _serviceName(TicketService service) => switch (service) {
      TicketService.electricity => 'Electricity',
      TicketService.water => 'Water',
      TicketService.property => 'Property'
    };
IconData _icon(TicketService service) => switch (service) {
      TicketService.electricity => LucideIcons.gauge,
      TicketService.water => LucideIcons.droplets,
      TicketService.property => LucideIcons.house
    };
NcrraStatusTone _tone(TicketStatus status) => switch (status) {
      TicketStatus.awaitingProvider => NcrraStatusTone.amber,
      TicketStatus.resolved => NcrraStatusTone.teal,
      _ => NcrraStatusTone.slate
    };
